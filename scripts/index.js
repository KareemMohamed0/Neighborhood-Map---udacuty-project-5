...
loadAsync('https://unpkg.com/leaflet@1.3.4/dist/leaflet.js',function(res1 , err1){ 
    if(err1) {
         alert(err1);
    }else{
        loadAsync('scripts/leaflet.smoothmarkerbouncing.js', function(res2 , err2){
        if(err2){
            alert(err2);
        }else{
        createMap();
...
function createMap(restureants) {
    ...
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png?callback=googleSuccess', {
